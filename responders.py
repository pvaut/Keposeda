...
def OpenDatabase():
    return MySQLdb.connect(host="localhost", user="root", passwd="1234", db='world')
...
def ReturnReflect(returndata):
    return returndata

#Return annotation information for a chromosome region
def ReturnAnnot(returndata):
...
def ReturnFullAnnotInfo(returndata):
    id=returndata['id']
...
def ReturnPagedQuery(returndata):
    mytablename=returndata['tbname']
    encodedquery=returndata['qry']
    myorderfield=returndata['order']
...
def ReturnRecordInfo(returndata):
    mytablename=returndata['tbname']
    encodedquery=returndata['qry']
